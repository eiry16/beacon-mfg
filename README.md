...
**中文数据集**（`data/gb/`，国标四级归档）：**23698 条记录**（其中 **15161 条电话已核实**，**8537 条待核实**），拆为 **23 个国标大类**（含未归类）：
...
| 39 计算机、通信和其他电子设备制造业 | 1440 | 966 | 474 |
| 40 仪器仪表制造业 | 38 | 25 | 13 |
| 42 废弃资源综合利用业 | 2 | 2 | 0 |
| 51 批发业 | 1755 | 1491 | 264 |
| 52 零售业 | 2 | 2 | 0 |

**英文数据集**（`data/en/gb/`，与中文同构的国标归档）：**23698 条英文镜像**，与中文按 `id` 一一对应（`en_backfill.py` 增量补齐 + `en_sync_industry.py` 补 `industry_en` 标签），供海外 Agent/买家使用。
