...
| `industry = null` | 无任何信号，或所属门类尚未登记 schema（建筑/运输/金融/房地产/商务服务/教育/卫生） | **不要替它猜**，按关键词检索 |

**归类三条红线**（与能力推断一致）：

1. **不硬贴标签**：拿不到信号就 `null`。宁可「未归类」，不可「错归类」。
2. **只信公司名**：公司名是最强证据；搜索关键词只能补位（`source="search_keyword"`，confidence 固定 `low`）。
3. **未登记门类留空**：建筑 E / 运输仓储 G / 金融 J / 房地产 K / 商务服务 L / 教育 P / 卫生 Q
   这些门类尚未登记 schema，命中的企业 `industry=null`，**不得**硬塞进已登记门类的代码。
   这不是「拒绝收录」——登记对应门类的 schema 后即可正常归类。

   > 2026-09-14 修订：原先这里写的是「粮食/食品/纺织/服装/木材/家具/造纸/印刷/水泥/石材
   > 等越界行业留空」。但那批行业**全是 C 制造业里真实存在的小类**（C13/C14/C17/C18/C19/
   > C20/C21/C22/C23/C26/C27/C30），属于已登记门类。因为「设计面窄」把真实企业判成 null，
   > 等于让客户 Agent 搜不到它们——是设计缺陷而不是数据缺陷。现在这些行业按真实国标码归类
   > （见 `scripts/classify_industry.py` 的 `EXTENDED_INDUSTRY_RULES`）。
   > 口径：**缺数据是抓取的问题，设计面不能减少。**
