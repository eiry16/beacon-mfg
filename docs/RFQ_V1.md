...
客户端直连模式下平台看不到任何一条询价：**谁发的、发给谁、投没投出去，全无记录**。
向真实企业发询价是带外部副作用的动作，不可审计就不能上线。中转路由补的正是这一点。

| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/v1/rfq` | 创建并投递。幂等键为 `rfq_id`，重投只回放存证 |
| GET | `/v1/rfq/{rfq_id}` | 查询状态与投递记录 |
| GET | `/v1/rfq` | 列表（按 supplier_id / buyer_id / status 过滤） |
| POST | `/v1/rfq/{rfq_id}/ack` | 登记厂商收讫（给 email/form/长连等异步通道回填） |

存证落在 `server/.data/rfq/{rfq_id}.json`，每条含：完整信封、状态机轨迹、
每次投递的时间与结果、调用方密钥指纹（**不明文存密钥**）。

四个刻意的设计选择：

1. **强制凭证。** 匿名投递无法追责，所以中转层要求 Bearer，未带凭证 401。
   存证里只留 `sha256(key)[:12]` 指纹——能追溯"同一个调用方"，又不会因日志泄露密钥。
2. **被拦截的询价也留档。** 触发安全护栏（非本机未确认）的询价照样写存证并落 `blocked`
   状态，否则"谁试图向哪家企业误发"这件事本身就不可审计。
3. **入口缺失要显式报错。** 厂商没声明 endpoint 且请求未指定时返回 400
   `NO_RFQ_ENDPOINT`，绝不静默丢弃——调用方必须知道询价没发出去。
4. **没配密钥就不签名。** `sig` 在 `RFQ_SIGNING_SECRET` 未配置时为 `null`。
   不为填字段而伪造一个签名出来。

`expired` 在查询时惰性计算：超过 `expires_at` 且仍无 ack → 状态转 `expired` 并落盘。
...
1. `buyer_secret` 的发放方式（目前 stub；中转层改用 `RFQ_SIGNING_SECRET` 环境变量）
