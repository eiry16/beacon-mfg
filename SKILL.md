...
- 当前 **8894 条电话已核实 + 5731 条待核实记录（合计 14625 条中文数据）**，另有英文镜像（见 `SKILL_EN.md` / `data/en/`）
...
- **国标行业覆盖**：14098/14625 条已归入 **47 个 GB/T 4754 小类**（527 条未归类），
  集中在通用设备制造业（4694）、金属制品业（4487）、橡胶和塑料制品业（1724）；
  另有 908 家落在 F51 批发业（`is_manufacturer=false`，贸易/批发商）
